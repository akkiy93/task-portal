...
フロントエンドで選択される「タスクの緊急度・重要度」は、以下のように優先度と重要度に変換されます：

| マトリクス値（フロントエンド） | 優先度（列E） | 重要度（列F） | 説明 |
|----------------------------|------------|------------|------|
| 高緊急/高重要（最優先） | 高 | 高 | 最優先で対応すべきタスク |
| 高緊急/低重要（急いで対応） | 高 | 低 | 急いで対応が必要だが、成果は限定的 |
| 低緊急/高重要（計画的に実行） | 低 | 高 | 計画的に実行すべき重要なタスク |
| 低緊急/低重要（後回し可） | 低 | 低 | 後回しにできるタスク |
...
| 列番号 | 列名 | 列インデックス | 説明 |
|--------|------|---------------|------|
| E (5) | 緊急度・重要度 | 4 | マトリクス値（高緊急/高重要、高緊急/低重要、低緊急/高重要、低緊急/低重要） |
